...
永続運用モードではSQLxのURL解釈に基づいてインメモリSQLiteを拒否し、接続後にも実ファイルを持つことをmigration前に検査します。表示側も実ファイルを必須とします。SQLite WALはネットワークファイルシステム向けではないため、DBは実行ホストのローカルディスクまたはDockerのローカルvolumeに置いてください。同じGuild・DB・tokenに対して複数のBotプロセスを同時起動しないでください。
