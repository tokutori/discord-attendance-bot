...
- [ ] Guildを切り替えた後、旧Guildの記録・プロフィール・表示・exportコマンドが拒否され、現在のDBが変わらない。
...
- [ ] 担当Guildのテキスト・アナウンス・フォーラムチャンネルを指定するとTopicを更新できる。別Guildや非対応種類を指定した場合、更新が拒否され、新しいActivityも公開されない。
- [ ] 更新先チャンネルを削除した場合や照会権限を失った場合も送信が中止される。設定を正しい宛先へ戻すと次の周期で更新が再開する。
- [ ] `names`を使う場合、Activityが担当Guild・Topic閲覧権限に限定されないことを理解し、Botが参加する他Guildを含む公開範囲を確認した。過去に公開したTopic・Activityは宛先検証失敗時も自動消去されない。
