...
pub async fn apply_due_auto_ends(
    ...
    for session in sessions {
        ...
        sqlx::query(
            "INSERT INTO attendance_auto_ends (
...
pub async fn latest_auto_ended(
    ...
    let Some(session) = sqlx::query_as::<_, AttendanceSession>(
        "SELECT s.* FROM attendance_sessions s
         INNER JOIN attendance_auto_ends a ON a.session_id = s.id
    ...
    let automatic_ended_at = sqlx::query_scalar::<_, i64>(
        "SELECT automatic_ended_at FROM attendance_auto_ends
...
pub async fn correct_auto_ended_session(
    ...
    .await?;
    sqlx::query("UPDATE attendance_auto_ends SET corrected_at = ? WHERE session_id = ?")
        .bind(now)
        .bind(id)
        .execute(&mut *tx)
        .await?;
...
pub async fn take_auto_end_notice(
    ...
    sqlx::query("UPDATE attendance_auto_ends SET notified_at = ? WHERE session_id = ?")
        .bind(now)
